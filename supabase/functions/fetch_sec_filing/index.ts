...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[${requestId}] ✅ Successfully fetched filing HTML (${rawHtml.length} characters)`);

    // Fix #2: Truncate HTML to 800,000 chars to prevent edge function memory/timeout issues
    // while still preserving the vast majority of the document content for parsing.
    const MAX_HTML_CHARS = 2_000_000;
    const html = rawHtml.length > MAX_HTML_CHARS ? rawHtml.slice(0, MAX_HTML_CHARS) : rawHtml;
    if (rawHtml.length > MAX_HTML_CHARS) {
      console.warn(`[${requestId}] ⚠️ HTML truncated from ${rawHtml.length} to ${MAX_HTML_CHARS} characters`);
    }
