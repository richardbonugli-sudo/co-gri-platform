...
import type { Tier1BacktestMetrics } from '@/services/yahooFinanceService';
...
interface TradingState {
  ...
  // Tier 1 backtest cache — persists across tab switches
  tier1Metrics: Tier1BacktestMetrics | null;
  tier1FetchedAt: number | null; // epoch ms
  setTier1Metrics: (metrics: Tier1BacktestMetrics | null) => void;
  setTier1FetchedAt: (ts: number | null) => void;
...
export const useTradingState = create<TradingState>()(
  ...
  tier1Metrics: null,
  tier1FetchedAt: null,
  setTier1Metrics: (metrics) => set({ tier1Metrics: metrics }),
  setTier1FetchedAt: (ts) => set({ tier1FetchedAt: ts }),
  ...
  runBacktest: async (config) => {
    ...
    {
      name: 'trading-tier1-cache',
      storage: createJSONStorage(() => localStorage),
      partialize: (state) => ({
        tier1Metrics: state.tier1Metrics,
        tier1FetchedAt: state.tier1FetchedAt,
      }),
    }
  )
);