 ...
 *
 * Data source: /docs/baseline-results/latest.json (fetched at runtime)
 * This file is written by the GitHub Actions workflow run-sec-baseline.yml
 * and committed to public/docs/baseline-results/latest.json automatically.
 */
...
// RunSummary schema — written by runSECBaseline.ts / GitHub Actions
interface RunSummaryResult {
  ticker: string;
  companyName: string;
  exchange: string;
  category: string;
...
interface RunSummary {
  ...
  results: RunSummaryResult[];
}

// BaselineData schema — internal representation for this component
...
// ─── Transform RunSummary → BaselineData ──────────────────────────────────────

function transformRunSummary(rs: RunSummary): BaselineData {
  const results = rs.results ?? [];
  return {
    runMeta: {
      runId: rs.runId ?? 'unknown',
      runDate: rs.startTime ?? new Date().toISOString(),
      phase: rs.phase ?? 'unknown',
      mode: 'LIVE',
      durationSeconds: Math.round((rs.durationMs ?? 0) / 1000),
      companiesEntered: rs.totalCompanies ?? results.length,
      companiesProcessed: rs.completedCompanies ?? results.length,
    },
    pipelineFunnel: {
      entered: results.length,
      secPathEntered: results.filter((r) => r.enteredSECPath).length,
      filingRetrieved: results.filter((r) => r.retrievalSucceeded).length,
      structuredParsed: results.filter((r) => r.structuredParsingSucceeded).length,
      materiallySpecific: results.filter((r) => r.materiallySpecific).length,
    },
    results: results.map((r) => ({
      ticker: r.ticker,
      category: (r.category as Category) ?? 'C',
      isADR: r.isADR ?? false,
      filingType: r.filingType ?? '10-K',
      filingDate: r.filingDate ?? r.timestamp ?? new Date().toISOString(),
      revenueTier: r.channelTiers?.revenue ?? 'NOT_RUN',
      supplyTier: r.channelTiers?.supply ?? 'NOT_RUN',
      assetsTier: r.channelTiers?.assets ?? 'NOT_RUN',
      financialTier: r.channelTiers?.financial ?? 'NOT_RUN',
      compositeConfidenceScore: r.compositeConfidenceScore ?? 0,
      confidenceGrade: r.confidenceGrade ?? 'F',
      isMateriallySpecific: r.materiallySpecific ?? false,
    })),
  };
}
...
export const BaselineResultsPanel: React.FC = () => {
  ...
  // ── Fetch data from public/docs/baseline-results/latest.json ──
  const loadData = async () => {
    setLoading(true);
    setFetchError(null);
    try {
      const res = await fetch('/docs/baseline-results/latest.json');
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const runSummary: RunSummary = await res.json();
      // Transform RunSummary schema → BaselineData schema
      const transformed = transformRunSummary(runSummary);
      setData(transformed.results.length > 0 ? transformed : null);
    } catch (e) {
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-slate-700/40 bg-slate-900/40 p-10 flex items-center justify-center gap-3">
        <RefreshCw className="h-5 w-5 animate-spin text-teal-400" />
        <span className="text-slate-400 text-sm">Loading baseline results…</span>
      </div>
    );
  }
  ...
  if (!hasData) {
    return (
      <div className="rounded-xl border border-slate-700/40 bg-slate-900/40 p-8 space-y-4">
        <div className="flex items-center gap-3 text-amber-400">
          <AlertCircle className="h-6 w-6 flex-shrink-0" />
          <p className="font-semibold text-base">Awaiting First GitHub Actions Run</p>
        </div>
        <p className="text-slate-400 text-sm leading-relaxed">
          No baseline data available yet. The results file will be created automatically after the
          first GitHub Actions workflow run completes and commits{' '}
          <span className="font-mono text-slate-300">docs/baseline-results/latest.json</span> to
          the repository.
        </p>
        ...
        <div className="space-y-3">
          <div className="rounded-md bg-slate-800 border border-slate-700 px-4 py-3">
            ...
            <ol className="text-sm text-slate-300 space-y-1 list-decimal list-inside">
              ...
              <li>Click the <span className="text-white font-semibold">Actions</span> tab</li>
              <li>Select <span className="text-blue-400 font-mono">SEC Runtime Baseline</span></li>
              <li>Click <span className="text-white font-semibold">Run workflow</span></li>
              <li>Choose a category (e.g. <span className="font-mono text-amber-400">A</span> for 41 pilot companies)</li>
          ...
          <div className="rounded-md bg-slate-800 border border-slate-700 px-4 py-3">
            ...
            <div className="font-mono text-sm text-slate-300 space-y-1">
              <p className="text-slate-500"># Run all Category A companies</p>
              <p>npm run sec-baseline</p>
              <p className="text-slate-500 mt-2"># Or run a single ticker</p>
