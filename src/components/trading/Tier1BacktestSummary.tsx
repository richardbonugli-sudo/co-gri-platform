...
export default function Tier1BacktestSummary() {
  ...
  useEffect(() => {
    // Consider cache stale if:
    // 1. No metrics at all
    // 2. Cache is older than TTL
    // 3. Metrics exist but strategy shows 0 return AND 0 tickers loaded (bad cached data)
    const isBadCache =
      tier1Metrics !== null &&
      tier1Metrics.strategyAnnReturn === 0 &&
      tier1Metrics.tickersLoaded === 0;
