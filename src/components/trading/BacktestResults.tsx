 ...
 *
 * Tier 1 Back-Test Pilot summary box is shown at the top of this panel.
 * It auto-runs on tab open with live Yahoo Finance data and displays
 * a performance comparison table (Strategy | S&P 500 | DJIA) plus
 * a Recharts equity curve.
 */
...
import { TrendingUp, TrendingDown, Download, Separator } from 'lucide-react';
...
// ── Tier 1 Pilot summary (always shown at top) ────────────────────────────────
import Tier1BacktestSummary from './Tier1BacktestSummary';
...
export default function BacktestResults() {
  ...
  return (
    <div className="space-y-6">
      {/* ── Tier 1 Back-Test Summary (prominent, always visible) ──────────── */}
      <Tier1BacktestSummary />
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-background px-2 text-muted-foreground">
            Custom Backtest (Configure &amp; Run Below)
          </span>
        </div>
      </div>

      {/* ── Custom backtest results (only shown after user runs one) ──────── */}
      {!backtestResult ? (
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            <p>No custom backtest results yet</p>
            <p className="text-sm mt-2">
              Configure backtest parameters in the panel on the left and click "Run Backtest"
            </p>
          </CardContent>
        </Card>
      ) : (
        <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <div className="text-2xl font-bold text-green-600">
                    +{backtestResult.performance.total_return.toFixed(1)}%
                  </div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Annualized Return</div>
                  <div className="text-2xl font-bold">
                    {backtestResult.performance.annualized_return.toFixed(1)}%
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="text-lg font-semibold">
                    {backtestResult.performance.volatility.toFixed(1)}%
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="text-lg font-semibold">{backtestResult.performance.total_trades}</div>
                </div>
                <div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Strategy vs {backtestResult.benchmark.benchmark_name}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <div className="text-xl font-bold text-green-600">
                    +{backtestResult.benchmark.strategy_return.toFixed(1)}%
                  </div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Benchmark Return</div>
                  <div className="text-xl font-bold">
                    {backtestResult.benchmark.benchmark_return.toFixed(1)}%
                  </div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Alpha</div>
                  <div className="text-xl font-bold text-green-600">
                    +{backtestResult.benchmark.alpha.toFixed(1)}%
                  </div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Beta</div>
                  <div className="text-lg font-semibold">{backtestResult.benchmark.beta.toFixed(2)}</div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Information Ratio</div>
                  <div className="text-lg font-semibold">
                    {backtestResult.benchmark.information_ratio.toFixed(2)}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Tracking Error</div>
                  <div className="text-lg font-semibold">
                    {backtestResult.benchmark.tracking_error.toFixed(1)}%
                  </div>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {backtestResult.attribution.by_sector.map((item) => (
              ...
              <div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {backtestResult.attribution.by_signal_type.map((item) => (
                      <TableRow key={item.signal_type}>
                        <TableCell>
                          <Badge
                            variant={item.signal_type === 'BUY' ? 'default' : 'destructive'}
                          >
