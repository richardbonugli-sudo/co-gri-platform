 ...
 * Covers all exchanges including the Phase-1 global expansion exchanges:
 *   BVMF (Brazil), HKG (Hong Kong), JSE (South Africa), LSE (UK),
 *   SGX (Singapore), TAI (Taiwan), TSE (Canada/Toronto)
 */
function normalizeExchange(fmpExchange: string, fmpShortName: string): string {
  ...
  const knownShortNames: Record<string, string> = {
    ...
    'TSX': 'TSX',
    'TSXV': 'TSXV',
    // Phase-1: Toronto Stock Exchange (FMP may return TSX or TORONTO)
    'TSE': 'TSE',
    'TORONTO': 'TSE',
    // UK
    'LSE': 'LSE',
    'LON': 'LSE',
    // Hong Kong — FMP may return HKSE, HKEX, or HKG
    'HKEX': 'HKG',
    'HKSE': 'HKG',
    'HKG': 'HKG',
    // Singapore
    'SGX': 'SGX',
    // Brazil — FMP may return SAO, BOVESPA, or B3
    'B3': 'BVMF',
    'SAO': 'BVMF',
    'BOVESPA': 'BVMF',
    'BVMF': 'BVMF',
    // Taiwan — FMP may return TWO, TWSE, or TAI
    'TWSE': 'TAI',
    'TWO': 'TAI',
    'TAI': 'TAI',
    // South Africa
...
/**
 * Return the Yahoo Finance-compatible ticker symbol for a given base ticker
 * and canonical exchange code (as stored in companyDatabase.ts).
 *
 * Rules:
 *  - BVMF / B3  → ticker + ".SA"
 *  - HKG / HKEX → zero-pad numeric tickers to 4 digits + ".HK"
 *  - JSE        → ticker + ".JO"
 *  - LSE        → ticker + ".L"  (pass-through if already ends with .L)
 *  - SGX        → ticker + ".SI"
 *  - TAI / TWSE → ticker + ".TW"
 *  - TSE / TSX  → ticker + ".TO" (special case: U.UN → U-UN.TO)
 *  - All others → ticker unchanged
 */
function getExchangeSuffix(exchange: string): string {
  switch (exchange.toUpperCase()) {
    ...
    case 'HKG':
    case 'HKEX':
      return '.HK'; // caller must handle zero-padding
    case 'JSE':
      return '.JO';
    case 'LSE':
      return '.L';
...
/**
 * Compute the Yahoo Finance-compatible ticker for a given ticker + exchange.
 * Handles zero-padding for HKG numeric tickers and the U.UN → U-UN.TO edge case.
 */
function getYahooFinanceTicker(ticker: string, exchange: string): string {
  ...
  if ((exc === 'TSE' || exc === 'TSX') && ticker === 'U.UN') {
    return 'U-UN.TO';
  }

  if (exc === 'LSE' && ticker.endsWith('.L')) {
    return ticker; // already suffixed
  }

  if ((exc === 'TSE' || exc === 'TSX') && ticker.endsWith('.TO')) {
    return ticker; // already suffixed
  }
...
function parseExistingDatabase(content: string): Company[] {
  ...
  for (const match of matches) {
    ...
    const adrEquivalent = extractStringField(block, 'adrEquivalent') || undefined;
    const companyTypeRaw = extractStringField(block, 'companyType');
    const companyType = (companyTypeRaw as Company['companyType']) || undefined;
    const yahooTicker = extractStringField(block, 'yahooTicker') || undefined;

    if (ticker) {
      companies.push({ ticker, name, exchange, country, sector, isADR, aliases, adrEquivalent, companyType, yahooTicker });
    }
  }
...
function generateTypeScriptFile(companies: Company[], lastUpdated: string): string {
  ...
  lines.push(`  /** For native-exchange listings that also have a US ADR, the ADR ticker */`);
  lines.push(`  adrEquivalent?: string;`);
  lines.push(`  /** Special classification for non-operating-company listings */`);
  lines.push(`  companyType?: 'operating' | 'commodity_trust' | 'gdr';`);
  lines.push(`  /** Yahoo Finance-compatible symbol (with exchange suffix & zero-padding) */`);
  lines.push(`  yahooTicker?: string;`);
  lines.push(`}`);
  lines.push(``);
 ...
 * Emits all fields including the Phase-1 additions: adrEquivalent,
 * companyType, and yahooTicker.
 */
function formatCompanyEntry(company: Company): string {
  ...
  if (company.adrEquivalent) {
    parts.push(`adrEquivalent: '${escapeSingleQuote(company.adrEquivalent)}'`);
  }

  if (company.companyType) {
    parts.push(`companyType: '${company.companyType}'`);
  }

  if (company.yahooTicker) {
    parts.push(`yahooTicker: '${escapeSingleQuote(company.yahooTicker)}'`);
  }
