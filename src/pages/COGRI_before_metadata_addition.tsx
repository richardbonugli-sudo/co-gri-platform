...
import { Loader2, Search, AlertCircle, ChevronDown, ChevronUp, Download, FileText, Lock, Unlock } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        {result && result.channelBreakdown && (
          <div className="max-w-6xl mx-auto space-y-6">
                ...
                <div className="text-center">
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="text-gray-300 text-sm">
                      <span className="font-semibold">{result.company}</span> ({result.symbol})
                    </div>
                    <div className="text-gray-400 text-xs">
                      Sector: {result.sector} | Multiplier: {result.sectorMultiplier.toFixed(2)}x
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="bg-[#0f1e2e] border-gray-700">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-2">
                  <span>📊</span>
                  <span>Step-by-Step Calculation Methodology</span>
                </CardTitle>
                <CardDescription className="text-gray-300">
                  Detailed breakdown of all mathematical formulas and calculations
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {/* Step 1: Channel Weight Calculation */}
                  <div className="bg-[#1a2332] p-4 rounded-lg border border-gray-700">
                    <div 
                      className="flex items-center justify-between cursor-pointer"
                      onClick={() => toggleChannelExpansion('step1')}
                    >
                      <h3 className="text-white font-bold text-lg flex items-center gap-2">
                        <span>Step 1:</span>
                        <span>Channel Weight Calculation</span>
                      </h3>
                      {expandedChannels['step1'] ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                    </div>
                    
                    {expandedChannels['step1'] && (
                      <div className="mt-4 space-y-3">
                        <div className="bg-blue-900/20 p-3 rounded border border-blue-700">
                          <div className="text-blue-300 font-mono text-sm mb-2">Formula:</div>
                          <div className="text-white font-mono text-xs">
                            Blended Weight = (Revenue × 0.40) + (Operations × 0.25) + (Supply × 0.35) + (Assets × 0.15)
                          </div>
                        </div>
                        <div className="text-gray-300 text-sm">
                          <strong>Coefficients:</strong>
                          <ul className="list-disc list-inside mt-2 space-y-1">
                            <li>Revenue: 40% - Primary revenue generation exposure</li>
                            <li>Operations: 25% - Manufacturing and operational facilities</li>
                            <li>Supply Chain: 35% - Critical supplier dependencies</li>
                            <li>Assets: 15% - Physical and financial asset concentration</li>
                          </ul>
                        </div>
                      </div>
                    )}
                  ...
                  {/* Step 2: Political Alignment Adjustment */}
                  <div className="bg-[#1a2332] p-4 rounded-lg border border-gray-700">
                    <div 
                      className="flex items-center justify-between cursor-pointer"
                      onClick={() => toggleChannelExpansion('step2')}
                    >
                      <h3 className="text-white font-bold text-lg flex items-center gap-2">
                        <span>Step 2:</span>
                        <span>Political Alignment Adjustment</span>
                      </h3>
                      {expandedChannels['step2'] ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                    </div>
                    
                    {expandedChannels['step2'] && (
                      <div className="mt-4 space-y-3">
                        <div className="bg-purple-900/20 p-3 rounded border border-purple-700">
                          <div className="text-purple-300 font-mono text-sm mb-2">Formula:</div>
                          <div className="text-white font-mono text-xs">
                            Adjustment Factor = 1.0 + 0.5 × (1.0 - Alignment Factor)
                          </div>
                        </div>
                        <div className="text-gray-300 text-sm">
                          <strong>Alignment Factors:</strong>
                          <ul className="list-disc list-inside mt-2 space-y-1">
                            <li>Same Country: 1.0 (no adjustment)</li>
                            <li>Allied: 0.9 (10% risk reduction)</li>
                            <li>Friendly: 0.8 (20% risk reduction)</li>
                            <li>Neutral: 0.7 (30% risk increase)</li>
                            <li>Competitive: 0.5 (50% risk increase)</li>
                            <li>Adversarial: 0.3 (70% risk increase)</li>
                          </ul>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="bg-[#1a2332] p-4 rounded-lg border border-gray-700">
                    <div 
                      className="flex items-center justify-between cursor-pointer"
                      onClick={() => toggleChannelExpansion('step3')}
                    >
                      <h3 className="text-white font-bold text-lg flex items-center gap-2">
                        <span>Step 3:</span>
                        <span>Country Risk Contribution</span>
                      </h3>
                      {expandedChannels['step3'] ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                    </div>
                    
                    {expandedChannels['step3'] && (
                      <div className="mt-4 space-y-3">
                        <div className="bg-orange-900/20 p-3 rounded border border-orange-700">
                          <div className="text-orange-300 font-mono text-sm mb-2">Formula:</div>
                          <div className="text-white font-mono text-xs">
                            Contribution = Blended Weight × CSI × Adjustment Factor
                          </div>
                        </div>
                        <div className="text-gray-300 text-sm">
                          Where CSI (Country Shock Index) ranges from 0-100 based on:
                          <ul className="list-disc list-inside mt-2 space-y-1">
                            <li>Political stability and governance quality</li>
                            <li>Economic volatility and sanctions exposure</li>
                            <li>Geopolitical tensions and conflict risk</li>
                  ...
                  <div className="bg-[#1a2332] p-4 rounded-lg border border-gray-700">
                    <div 
                      className="flex items-center justify-between cursor-pointer"
                      onClick={() => toggleChannelExpansion('step4')}
                    >
                      <h3 className="text-white font-bold text-lg flex items-center gap-2">
                        <span>Step 4:</span>
                        <span>Final Score Calculation</span>
                      </h3>
                      {expandedChannels['step4'] ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                    </div>
                    
                    {expandedChannels['step4'] && (
                      <div className="mt-4 space-y-3">
                        <div className="bg-green-900/20 p-3 rounded border border-green-700">
                          ...
                          <div className="text-white font-mono text-xs">
                            Final Score = Σ(All Country Contributions) × Sector Multiplier
                          </div>
                        </div>
                        <div className="text-gray-300 text-sm">
                          <strong>For this assessment:</strong>
                          <ul className="list-disc list-inside mt-2 space-y-1">
                            <li>Raw Score: {result.rawScore.toFixed(2)}</li>
                            <li>Sector Multiplier: {result.sectorMultiplier.toFixed(2)}x ({result.sector})</li>
                            <li>Final Score: {result.geopoliticalRiskScore}</li>
                            <li>Risk Level: {result.riskLevel}</li>
                          </ul>
                        </div>
                      </div>
                    )}
            ...
            {/* Detailed Country Risk Exposures */}
            <Card className="bg-[#0f1e2e] border-gray-700">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-2">
                  <span>🌍</span>
                  <span>Detailed Country Risk Exposures</span>
                </CardTitle>
                <CardDescription className="text-gray-300">
                  Complete breakdown of all country exposures with evidence classification
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {result.countryExposures
                    ...
                    .map((exp, index) => {
                      ...
                      const isExpanded = expandedCountries[exp.country];
                      
                      return (
                        <div key={exp.country} className="bg-[#1a2332] rounded-lg border border-gray-700 overflow-hidden">
                          <div 
                            className="p-4 cursor-pointer hover:bg-[#1f2937] transition-colors"
                            onClick={() => toggleCountryExpansion(exp.country)}
                          >
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="flex items-center gap-2">
                                    <span className="text-white font-bold text-lg">{exp.country}</span>
                                    {exp.fallbackType && (
                                      <span className={`px-2 py-0.5 rounded text-xs font-semibold border ${getFallbackTypeBadgeColor(exp.fallbackType)}`}>
                                        {getFallbackTypeIcon(exp.fallbackType)} {exp.fallbackType}
                                      </span>
                                    )}
                                    {exp.politicalAlignment && (
                                      <span className="text-sm">
                                        {getAlignmentIcon(exp.politicalAlignment.relationship)}
                                      </span>
                                    )}
                                  </div>
                                  <div className="flex items-center gap-4 mt-1 text-sm">
                                    <span className="text-gray-400">
                                      Exposure: <span className="text-white font-semibold">{(exp.exposureWeight * 100).toFixed(2)}%</span>
                                    </span>
                                    <span className="text-gray-400">
                                      CSI: <span className="text-white font-semibold">{exp.countryShockIndex}</span>
                                    </span>
                                    <span className="text-gray-400">
                                      Contribution: <span className="text-orange-400 font-semibold">{exp.contribution.toFixed(2)}</span>
                                    </span>
                                  </div>
                                </div>
                              </div>
                          ...
                          {isExpanded && channelData && (
                            <div className="px-4 pb-4 space-y-4 border-t border-gray-700">
                              ...
                              {exp.politicalAlignment && (
                                <div className="mt-4 bg-purple-900/20 p-3 rounded border border-purple-700">
                                  <h4 className="text-purple-300 font-semibold mb-2 flex items-center gap-2">
                                    <span>{getAlignmentIcon(exp.politicalAlignment.relationship)}</span>
                                    <span>Political Alignment</span>
                                  </h4>
                                  <div className="grid grid-cols-3 gap-3 text-sm">
                                    <div>
                                      <div className="text-gray-400">Relationship</div>
                                      <div className="text-white font-semibold capitalize">{exp.politicalAlignment.relationship}</div>
                                    </div>
                                    <div>
                                      <div className="text-gray-400">Alignment Factor</div>
                                      <div className="text-white font-semibold">{exp.politicalAlignment.alignmentFactor.toFixed(2)}</div>
                                    </div>
                                    <div>
                                      <div className="text-gray-400">Risk Adjustment</div>
                                      <div className="text-white font-semibold">
                                        {((1.0 + 0.5 * (1.0 - exp.politicalAlignment.alignmentFactor)) * 100 - 100).toFixed(0)}%
                                      </div>
                                    </div>
                                  </div>
                                  <div className="mt-2 text-xs text-gray-400">
                                    Source: {exp.politicalAlignment.source}
                                  </div>
                                </div>
                              )}
                              ...
                              <div className="mt-4 bg-red-900/20 p-3 rounded border border-red-700">
                                ...
                                <div className="text-sm text-gray-300 space-y-1">
                                  <div>Blended Weight: {(exp.exposureWeight * 100).toFixed(2)}%</div>
                                  <div>Country Shock Index (CSI): {exp.countryShockIndex}</div>
                                  <div>Political Adjustment: {exp.politicalAlignment ? (1.0 + 0.5 * (1.0 - exp.politicalAlignment.alignmentFactor)).toFixed(2) : '1.00'}x</div>
                                </div>
                                <div className="text-sm text-gray-300 font-mono mt-2">
                                  = {(exp.exposureWeight * 100).toFixed(2)}% × {exp.countryShockIndex} × {exp.politicalAlignment ? (1.0 + 0.5 * (1.0 - exp.politicalAlignment.alignmentFactor)).toFixed(2) : '1.00'}
                                </div>
                                <div className="text-white font-bold text-xl mt-2">
                                  = {exp.contribution.toFixed(2)} points
                                </div>
                              </div>
                            </div>
                          )}
