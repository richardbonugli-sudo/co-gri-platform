...
import { Loader2, Search, AlertCircle, ChevronDown, ChevronUp, Download, FileText, Lock, Unlock, CheckCircle, Shield, Star, AlertTriangle } from 'lucide-react';
...
// Helper function to get evidence badge components
const getEvidenceBadges = (fallbackType?: FallbackType, status?: EvidenceStatus) => {
  if (status === 'evidence' || fallbackType === 'none') {
    return ['■', '■']; // Green squares - high confidence
  } else if (fallbackType === 'SSF') {
    return ['●', '●']; // Blue circles - sector validated
  } else if (fallbackType === 'RF') {
    return ['■', '●']; // Mixed - medium confidence
  } else {
    return ['◆', '◆']; // Other - lower confidence
  }
};
...
// Helper function to get alignment icon component
...
// Helper function to get relationship detail description
...
// Helper function to get business implications
...
// Helper function to generate Step 3 CSI details
...
// Helper function to generate Step 4 Political Alignment details
const generateStep4PoliticalAlignmentDetails = (
  countryExposures: CountryExposure[]
): string => {
  let details = `\n\n📊 v3.4 Enhanced Detailed Breakdown:\n\n`;
  
  // PART 1: Data Source Documentation
  details += `${'═'.repeat(79)}\n\n`;
  ...
  // PART 4: Country-by-Country Calculations
  details += `${'═'.repeat(79)}\n\n`;
...
// Helper function to generate Step 5 Aggregation details
...
// Helper function to generate Step 6 Sector Adjustment details
...
export default function COGRI() {
  ...
  const handleSearch = async (searchTicker?: string) => {
    ...
    try {
      ...
      // Generate all 7 calculation steps
      ...
      // Step 3: Country Shock Index with detailed country-by-country CSI breakdown
      ...
      // Step 4: Weighted Risk Contribution with Political Alignment Analysis
      ...
      // Step 5: Raw CO-GRI Score Aggregation
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        {result && result.channelBreakdown && (
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            {/* v3.4 Enhanced Four-Channel Exposure Analysis Table */}
