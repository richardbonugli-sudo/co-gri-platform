...
export default function COGRI() {
  ...
  const handleSearch = async (searchTicker?: string) => {
    ...
    try {
      ...
      calculationSteps.push({
        step: 'Step 2: Normalization of Exposure Weights',
        formula: 'W_normalized = W_i,c / Σ(W_i,c)',
        values: {
          'Sum of Pre-Normalized Weights': totalExposurePreNorm,
          'Normalization Factor': 1.0 / totalExposurePreNorm,
          'Countries After Filtering': countryExposures.length
        },
        result: 1.0,
        explanation: `Normalized ${countryExposures.length} country exposure weights to sum to 100%`
      });
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        {result && result.channelBreakdown && (
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            {/* I'm truncating here for brevity, but the rest of the JSX continues unchanged */}
          </div>
        )}
      </div>
    </div>
