 ...
 *
 * Fix 4: Also calls parseNarrativeText to expand the admissible set with:
 *   - Regional definitions (e.g. "Asia Pacific includes China, Vietnam, Malaysia...")
 *   - Supply-channel country mentions from the narrative parser
 *   Signal strength is upgraded to at least 'medium' if regional definitions are found.
 */
export function buildAdmissibleSetFromNarrative(
  ...
  // Step A: extractSupplyCountriesFromNarrative (existing logic)
  ...
  let admissibleSet = new Set<string>(signals.explicitCountries);

  for (const region of signals.regions) {
    const regionMembers = getRegionMembersFromName(region);
    regionMembers.forEach(c => admissibleSet.add(c));
  }

  // Fix 4 Step B: parseNarrativeText to capture regional definitions and supply mentions
  let signalStrength = signals.signalStrength;
  try {
    const narrativeResult = parseNarrativeText(narrative);

    if (narrativeResult.regionalDefinitions.length > 0) {
      console.log(`[V5 Admissible Set] Fix 4: Found ${narrativeResult.regionalDefinitions.length} regional definitions`);
      for (const def of narrativeResult.regionalDefinitions) {
        def.countries.forEach(c => admissibleSet.add(c));
      }
      // Upgrade signal strength to at least 'medium' when regional definitions are present
      if (signalStrength === 'none' || signalStrength === 'low') {
        signalStrength = 'medium';
        console.log(`[V5 Admissible Set] Fix 4: Signal strength upgraded to 'medium' (regional definitions found)`);
      }
    }

    // Add supply-channel country mentions from narrative parser
    const supplyMentions = narrativeResult.countryMentions.filter(m => m.channel === 'supply');
    if (supplyMentions.length > 0) {
      console.log(`[V5 Admissible Set] Fix 4: Adding ${supplyMentions.length} supply-channel country mentions`);
      supplyMentions.forEach(m => admissibleSet.add(m.country));
    }
  } catch (narrativeError) {
    console.warn(`[V5 Admissible Set] Fix 4: parseNarrativeText failed (non-critical):`, narrativeError);
  }

  let admissibleArray = Array.from(admissibleSet);

  if (signals.excludesHomeCountry) {
    admissibleArray = admissibleArray.filter(c => c !== homeCountry);
    console.log(`[V5 Admissible Set] Excluded home country ${homeCountry} (not in narrative)`);
  }

  console.log(`[V5 Admissible Set] Final admissible set (${admissibleArray.length} countries): ${admissibleArray.join(', ')}`);

  return {
    admissibleSet: admissibleArray,
