...
import {
  fetchFromFMP,
  triangulateGeographicSegments,
  type TriangulationResult,
} from './financialDataAPI';

// ============================================================================
// FILING RECENCY UTILITIES (P3-A)
// ============================================================================
 ...
 */
export function filingRecencyMultiplier(filingDateISO: string | null): number {
...
/**
 * Returns a human-readable label for the filing recency multiplier.
 */
export function filingRecencyLabel(filingDateISO: string | null): string {
  const m = filingRecencyMultiplier(filingDateISO);
  if (m === 1.00) return 'Current';
  if (m === 0.85) return '1 year stale';
  if (m === 0.70) return '2 years stale';
  return 'Significantly stale — review recommended';
}
import {
  isKnownRegion,
  getRegionCountries,
...
export interface IntegratedExposureData {
  ...
  /** P3-B: FMP geographic segment cross-reference + triangulation result. */
  fmpCrossReference?: FMPCrossReferenceResult;
}
...
// ============================================================================
// P3-B: FMP CROSS-REFERENCE RESULT (attached to IntegratedExposureData)
// ============================================================================

/**
 * Subset of TriangulationResult stored on IntegratedExposureData.
 * Consumers can inspect per-country agreement without importing financialDataAPI.
 */
export interface FMPCrossReferenceResult {
  /** Number of channels confirmed by FMP (0–4). ≥2 triggers the fmpBoost in computeCompositeConfidence. */
  fmpConfirmedChannels: number;
  /** Overall agreement score across all triangulated countries (0–1). */
  overallAgreement: number;
  /** Source of FMP data used. */
  fmpDataSource: 'fmp_api' | 'cache' | 'unavailable';
  /** Top-5 triangulated countries for display / audit. */
  topCountries: Array<{
    country: string;
    mergedWeight: number;
...
export async function integrateStructuredData(
  ...
  // P3-A: Apply filing recency multiplier to all channel entry confidence values.
  // filingDate is stored on the ParsedSECData object returned by parseSECFiling().
  const filingDateISO: string | null = (secData as (ParsedSECData & { filingDate?: string }) | null)?.filingDate ?? null;
  const recencyMult = filingRecencyMultiplier(filingDateISO);

  if (recencyMult < 1.00) {
    const recencyLabel = filingRecencyLabel(filingDateISO);
    console.log(`[Structured Data Integration] ⏱ Filing recency multiplier: ${recencyMult} (${recencyLabel})`);

    const applyRecency = (channel: Record<string, IntegratedChannelData>): void => {
      for (const entry of Object.values(channel)) {
        // confidence defaults to 100 if not explicitly set
        const base = (entry as IntegratedChannelData & { confidence?: number }).confidence ?? 100;
        (entry as IntegratedChannelData & { confidence?: number }).confidence = Math.round(base * recencyMult);
      }
    };

    applyRecency(revenueResult.channel);
    applyRecency(supplyResult.channel);
    applyRecency(assetsResult.channel);
    applyRecency(financialResult.channel);
  }
  ...
  // ── P3-B: FMP Geographic Cross-Reference + Triangulation ──────────────────
  const fmpCrossRef = await crossReferenceWithFMP(ticker, result);
  if (fmpCrossRef) {
    result.fmpCrossReference = fmpCrossRef;
    console.log(
      `[Structured Data Integration] FMP Cross-Reference: ` +
      `fmpConfirmedChannels=${fmpCrossRef.fmpConfirmedChannels}, ` +
      `overallAgreement=${fmpCrossRef.overallAgreement.toFixed(2)}, ` +
      `source=${fmpCrossRef.fmpDataSource}`
    );
  }
...
// ============================================================================
// P3-B: FMP CROSS-REFERENCE HELPER
// ============================================================================

/**
 * Cross-references the revenue channel of an already-integrated result against
 * FMP API geographic segment data, producing a `FMPCrossReferenceResult`.
 *
 * Steps:
 *   1. Build a SEC revenue weight map from `integratedData.revenueChannel`.
 *   2. Fetch FMP data for the ticker (uses cache if available).
 *   3. Call `triangulateGeographicSegments` to merge both sources.
 *   4. Map the `TriangulationResult` to `FMPCrossReferenceResult`.
 *
 * Returns `null` if neither SEC revenue data nor FMP data is available.
 */
async function crossReferenceWithFMP(
  ticker: string,
  integratedData: IntegratedExposureData
): Promise<FMPCrossReferenceResult | null> {
  const tag = `[FMP CrossRef:${ticker}]`;

  // ── Build SEC revenue weight map from the revenue channel ─────────────────
  const secSegments = new Map<string, number>();
  for (const [country, channelData] of Object.entries(integratedData.revenueChannel)) {
    const w = (channelData as IntegratedChannelData).weight;
    if (typeof w === 'number' && w > 0) {
      secSegments.set(country, w);
    }
  }
  ...
  let fmpData = null;
  try {
    fmpData = await fetchFromFMP(ticker);
  } catch (err) {
    console.warn(`${tag} FMP fetch failed:`, err);
  }

  // If we have neither source, skip triangulation
  if (secSegments.size === 0 && fmpData === null) {
    console.log(`${tag} No SEC revenue data and no FMP data — skipping triangulation.`);
    return null;
  }

  // ── Triangulate ────────────────────────────────────────────────────────────
  const tri: TriangulationResult = triangulateGeographicSegments(ticker, secSegments, fmpData);

  // ── Map to FMPCrossReferenceResult ─────────────────────────────────────────
  const topCountries = tri.countries.slice(0, 5).map(e => ({
    country: e.country,
    mergedWeight: e.mergedWeight,
    secWeight: e.secWeight,
    fmpWeight: e.fmpWeight,
    agreementScore: e.agreementScore,
    fmpConfirmed: e.fmpConfirmed,
  }));

  return {
    fmpConfirmedChannels: tri.fmpConfirmedChannels,
    overallAgreement: tri.overallAgreement,
    fmpDataSource: tri.fmpDataSource,
    topCountries,
    triangulatedAt: tri.triangulatedAt,
  };
}
