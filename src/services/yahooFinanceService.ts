 ...
 *   - All bar dates normalised to Monday of their ISO week before alignment
 ...
 *   - Win rate: % of weeks (aligned) strategy outperforms S&P 500
...
/**
 * Normalise a Date to the Monday of its ISO week (UTC).
 *
 * Yahoo Finance returns weekly bars with varying weekday timestamps depending
 * on the ticker and data source (some use Monday open, some use Friday close,
 * some use the exact trading day).  Normalising every bar to the Monday of its
 * week ensures that price lookups across different tickers align correctly.
 */
function toMonday(date: Date): Date {
  const d = new Date(date);
  // getDay(): 0=Sun, 1=Mon, …, 6=Sat
  const day = d.getUTCDay();
  // Days to subtract to reach Monday (Sun→+1 day forward is wrong; use mod)
  const diff = day === 0 ? -6 : 1 - day; // negative = go back
...
function parseYahooResponse(ticker: string, data: unknown): TickerPriceData {
  ...
  // Deduplicate by Monday-normalised week: keep the last bar per week
  const weekMap = new Map<number, WeeklyBar>();
  for (let i = 0; i < timestamps.length; i++) {
    const c = closes[i];
    const ac = adjCloses[i] ?? c;
    if (c == null || isNaN(c) || c <= 0) continue;
    const monday = toMonday(new Date(timestamps[i] * 1000));
    weekMap.set(monday.getTime(), {
      date: monday,
      close: c,
  ...
  const bars: WeeklyBar[] = Array.from(weekMap.values()).sort(
 ...
 * Dates are normalised to Monday of their ISO week for alignment.
 */
export async function loadBenchmarkFromStaticCsv(
  ...
  for (const csvPath of candidates) {
    try {
      ...
      // Deduplicate by Monday-normalised week
      const weekMap = new Map<number, WeeklyBar>();
 ...
 * Dates are normalised to Monday of their ISO week.
 */
function generateSyntheticBenchmark(
  ...
  const current = toMonday(new Date(startDate));
...
/**
 * Annualise from an array of weekly log-returns.
 *
 * @param weeklyLogReturns  Active weekly log-returns (non-null weeks only).
 * @param totalWeeks        Full calendar weeks in the backtest period.
 */
function annualiseFromLogReturns(
  weeklyLogReturns: number[],
  totalWeeks: number
): number {
  if (weeklyLogReturns.length === 0 || totalWeeks === 0) return 0;
  const totalLog = weeklyLogReturns.reduce((s, r) => s + r, 0);
  const annLog = totalLog * (52 / totalWeeks);
  return (Math.exp(annLog) - 1) * 100;
}

/**
 * Annualised Sharpe ratio from weekly log-returns.
 */
function sharpeRatio(weeklyLogReturns: number[], totalWeeks: number): number {
  if (weeklyLogReturns.length < 4 || totalWeeks === 0) return 0;
  const n = weeklyLogReturns.length;
  const meanActive = weeklyLogReturns.reduce((s, r) => s + r, 0) / n;
  const meanPerCalendarWeek = meanActive * (n / totalWeeks);
  const variance =
    weeklyLogReturns.reduce((s, r) => s + (r - meanActive) ** 2, 0) / (n - 1);
  const stdDev = Math.sqrt(variance);
  if (stdDev === 0) return 0;
  return ((meanPerCalendarWeek - RISK_FREE_WEEKLY) / stdDev) * Math.sqrt(52);
}
...
function buildMetrics(
  ...
  // ── FIX 1: Build Monday-keyed price lookup for every ticker ───────────────
  // All bars were already normalised to Monday in parseYahooResponse /
  // loadBenchmarkFromStaticCsv / generateSyntheticBenchmark, so
  // bar.date.getTime() is already a Monday timestamp.  We just build the map.
  ...
  // ── FIX 4: Date-range guard — trim to common overlap window ───────────────
  // Find the latest first-bar date and earliest last-bar date across ALL tickers
  // (strategy + benchmarks) so the equity curve only covers weeks where every
  // series has data.
  const allSeries = [
    spData,
    djData,
    ...loadedTickers.map(t => priceMap.get(t)!),
  ];

  let maxStart = spData.bars[0].date.getTime();
  let minEnd   = spData.bars[spData.bars.length - 1].date.getTime();

  for (const series of allSeries) {
    const first = series.bars[0].date.getTime();
    const last  = series.bars[series.bars.length - 1].date.getTime();
    if (first > maxStart) maxStart = first;
    if (last  < minEnd)   minEnd   = last;
  }

  // Build master timeline from S&P 500 bars trimmed to [maxStart, minEnd]
  const masterDates = spData.bars
    .map(b => b.date)
    .filter(d => d.getTime() >= maxStart && d.getTime() <= minEnd);

  if (masterDates.length < 4) {
    // Fallback: use full SP500 timeline if overlap is too short
    masterDates.length = 0;
    spData.bars.forEach(b => masterDates.push(b.date));
  }

  const totalWeeks = masterDates.length - 1;

  // ── FIX 2 & 3: Aligned weekly array for strategy + benchmarks ────────────
  // For each master date step i, record strategy log-return (null if no tickers
  // contributed) and benchmark log-returns.
  interface AlignedWeek {
    strategyLogReturn: number | null;
    spLogReturn: number;
    djLogReturn: number;
  }
  const alignedWeeks: AlignedWeek[] = [];

  // Build SP and DJ lookups (already Monday-keyed)
  const spLookup = new Map<number, number>();
  for (const bar of spData.bars) spLookup.set(bar.date.getTime(), bar.adjClose);
  const djLookup = new Map<number, number>();
  for (const bar of djData.bars) djLookup.set(bar.date.getTime(), bar.adjClose);

  // Initialise prev prices at the first master date
  const prevPrices = new Map<string, number>();
  for (const t of loadedTickers) {
    // Find the closest bar at or before masterDates[0]
    const lookup = priceLookup.get(t)!;
    const firstTs = masterDates[0].getTime();
    let initPrice = lookup.get(firstTs);
    if (initPrice == null) {
      // Use the first available bar price as initialisation
      const firstBar = priceMap.get(t)!.bars[0];
      initPrice = firstBar.adjClose;
    }
    prevPrices.set(t, initPrice);
  }

  let prevSP = spLookup.get(masterDates[0].getTime()) ?? spData.bars[0].adjClose;
  let prevDJ = djLookup.get(masterDates[0].getTime()) ?? djData.bars[0].adjClose;
  ...
  for (let i = 1; i < masterDates.length; i++) {
    ...
    // ── FIX 2: Direct log-returns for strategy ────────────────────────────
    let sumLogReturn = 0;
    let count = 0;
    for (const t of loadedTickers) {
      ...
      if (curr != null && curr > 0 && prev > 0) {
        sumLogReturn += Math.log(curr / prev);
        prevPrices.set(t, curr);
        count++;
      }
    }

    let strategyLogReturn: number | null = null;
    if (count > 0) {
      strategyLogReturn = sumLogReturn / count;
      strategyEq *= Math.exp(strategyLogReturn);
    }

    // ── FIX 2: Direct log-returns for benchmarks ──────────────────────────
    const spCurr = spLookup.get(ts) ?? prevSP;
    const spLogReturn = prevSP > 0 && spCurr > 0 ? Math.log(spCurr / prevSP) : 0;
    spEq *= Math.exp(spLogReturn);
    prevSP = spCurr;

    const djCurr = djLookup.get(ts) ?? prevDJ;
    const djLogReturn = prevDJ > 0 && djCurr > 0 ? Math.log(djCurr / prevDJ) : 0;
    djEq *= Math.exp(djLogReturn);
    prevDJ = djCurr;

    alignedWeeks.push({ strategyLogReturn, spLogReturn, djLogReturn });
  ...
  // ── FIX 3: Extract aligned return arrays ─────────────────────────────────
  // Strategy: only non-null weeks (weeks where ≥1 ticker contributed)
  const strategyWeeklyLogReturns = alignedWeeks
    .filter(w => w.strategyLogReturn !== null)
    .map(w => w.strategyLogReturn as number);

  // Benchmarks: all weeks (dense)
  const spWeeklyLogReturns    = alignedWeeks.map(w => w.spLogReturn);
  const djWeeklyLogReturns    = alignedWeeks.map(w => w.djLogReturn);

  // Win rate: compare only weeks where strategy has a non-null return,
  // using the SP return for the SAME aligned index.
  let wins = 0;
  let comparisons = 0;
  for (const w of alignedWeeks) {
    if (w.strategyLogReturn !== null) {
      comparisons++;
      if (w.strategyLogReturn > w.spLogReturn) wins++;
    }
  }
  const winRate = comparisons > 0 ? (wins / comparisons) * 100 : 0;

  // ── Compute final metrics ─────────────────────────────────────────────────
  const strategyAnnReturn = annualiseFromLogReturns(strategyWeeklyLogReturns, totalWeeks);
  const spAnnReturn       = annualiseFromLogReturns(spWeeklyLogReturns,       totalWeeks);
  const djAnnReturn       = annualiseFromLogReturns(djWeeklyLogReturns,       totalWeeks);

  const strategySharpe = sharpeRatio(strategyWeeklyLogReturns, totalWeeks);
  const spSharpe       = sharpeRatio(spWeeklyLogReturns,       totalWeeks);
  const djSharpe       = sharpeRatio(djWeeklyLogReturns,       totalWeeks);
  ...
  // ── FIX 5: Synthetic ticker warning ──────────────────────────────────────
  if (dataSource === 'estimated') {
    const syntheticTickers = strategyTickers.filter(t => {
      const data = priceMap.get(t);
      // Synthetic tickers have perfectly uniform bar spacing (generated from SP bars)
      // We detect them by checking if their first bar date exactly matches SP's first bar
      if (!data) return false;
      return data.bars[0].date.getTime() === spData.bars[0].date.getTime() &&
             data.bars.length === spData.bars.length;
    });
    if (syntheticTickers.length > 0) {
      console.warn(
        `[CO-GRI Backtest] ⚠️  SYNTHETIC DATA WARNING: The following tickers are using ` +
        `synthetic/estimated price data — results are inflated and NOT representative of ` +
        `real market performance: ${syntheticTickers.join(', ')}. ` +
        `Deploy the Supabase Edge Function or use a CORS-permissive environment for real data.`
      );
    }
  }
...
export async function runTier1Backtest(
  ...
  console.info(
    `[CO-GRI Backtest] Strategy tickers loaded: ${loadedStrategyTickers.length}/${tickers.length}. ` +
    `DataSource so far: ${dataSource}.`
  );

  // Helper: generate synthetic price data for a strategy ticker aligned to SP500 dates
  const generateSyntheticTicker = (ticker: string, spBars: WeeklyBar[]): TickerPriceData => {
    let seed = 0;
    ...
    const weeklyDrift = Math.log(1.20) / 52;
    const weeklyVol = 0.18 / Math.sqrt(52);
    ...
    for (const spBar of spBars) {
      if (bars.length === 0) {
        bars.push({ date: new Date(spBar.date.getTime()), close: price, adjClose: price });
      } else {
        const logReturn = weeklyDrift + weeklyVol * randn();
        weeklyReturns.push(logReturn);
        price = price * Math.exp(logReturn);
  ...
  if (loadedStrategyTickers.length === 0) {
    console.warn(
      `[CO-GRI Backtest] No real strategy ticker data loaded. ` +
      `Generating synthetic CO-GRI strategy data (~20% ann. return, ~18% vol).`
    );
    const spBars = priceMap.get('^GSPC')!.bars;
    for (const ticker of tickers.slice(0, Math.min(tickers.length, 15))) {
      priceMap.set(ticker, generateSyntheticTicker(ticker, spBars));
    }
    dataSource = 'estimated';
  } else if (loadedStrategyTickers.length < 10 && dataSource === 'live') {
    console.warn(
      `[CO-GRI Backtest] Only ${loadedStrategyTickers.length} live tickers. ` +
      `Supplementing with synthetic data for missing tickers.`
    );
    const spBars = priceMap.get('^GSPC')!.bars;
    for (const ticker of failedTickers.slice(0, Math.min(failedTickers.length, 15))) {
      priceMap.set(ticker, generateSyntheticTicker(ticker, spBars));
    }
    dataSource = 'estimated';
  }
