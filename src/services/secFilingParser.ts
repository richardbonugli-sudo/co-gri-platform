 ...
 * 
 * CHANGELOG:
 * - 2026-04-23: Fix 1 — wire supplyChainNarrativeContext / revenueNarrativeContext / ppeNarrativeContext
 * - 2026-04-23: Fix 2 — propagate LLM extraction context to narrative context fields
 * - 2026-04-23: Added failureCategory field to ParsedSECData
...
export interface ParsedSECData {
  ...
  /**
   * Fix 1: Failure category for diagnostics.
   * Set at each failure/success point in parseSECFiling().
   */
  failureCategory?:
    | 'cik_not_found'
    | 'filing_not_found'
    | 'html_empty'
    | 'parsing_success_no_tables'
    | 'parsing_success_with_tables';
}
...
// ============================================================================
// NARRATIVE CONTEXT EXTRACTORS (Fix 1 & Fix 2)
// ============================================================================

/**
 * Extract supply-chain narrative context from SEC filing HTML.
 *
 * Fix 1: Searches for sentences containing supply-chain keywords across the full
 * body text AND specifically within Item 1 (Business) and Item 1A (Risk Factors).
 * Returns up to 8,000 characters of concatenated matching sentences.
 * Assigned to result.supplyChainNarrativeContext in parseSECFiling().
 */
export function extractSupplyChainNarrativeContext(html: string): string {
  const $ = cheerio.load(html);
  const supplyKeywords = [
    'supplier', 'supply chain', 'manufacturing partner', 'contract manufacturer',
    'component', 'raw material', 'sourcing', 'procurement', 'assembly', 'outsourc'
  ];
  ...
  function scanText(text: string) {
    // Split on sentence boundaries (lookbehind for punctuation)
  ...
  // 1. Scan Item 1 (Business) and Item 1A (Risk Factors) sections first
  $('body').find('*').each((_, element) => {
    const text = $(element).text();
    if (/Item\s+1[Aa]?\b.*?(Business|Risk\s+Factor)/i.test(text) && text.length < 500) {
      let current = $(element).next();
      let count = 0;
      while (current.length > 0 && count < 30) {
        scanText(current.text());
        current = current.next();
        count++;
      }
    }
  ...
  // 2. Scan full body text for any remaining supply-keyword sentences
  scanText($('body').text());

  const result = matchedSentences.join(' ').substring(0, 8000);
  if (result.length > 0) {
    console.log(`[SEC Parser] ✅ extractSupplyChainNarrativeContext: ${result.length} chars`);
  }
  return result;
}

/**
 * Extract revenue narrative context from SEC filing HTML.
 *
 * Fix 1: Searches for sentences containing revenue/geographic keywords.
 * Returns up to 8,000 characters.
 * Assigned to result.revenueNarrativeContext in parseSECFiling().
 */
export function extractRevenueNarrativeContext(html: string): string {
  const $ = cheerio.load(html);
  const revenueKeywords = [
    'revenue', 'net sales', 'geographic', 'region', 'segment', 'market',
    'customers', 'demand', 'international', 'domestic'
  ];
  ...
  function scanText(text: string) {
    ...
    for (const sentence of sentences) {
      const lower = sentence.toLowerCase();
      if (revenueKeywords.some(kw => lower.includes(kw))) {
        const trimmed = sentence.trim();
        if (trimmed.length > 20 && !seen.has(trimmed)) {
          seen.add(trimmed);
          matchedSentences.push(trimmed);
  ...
  const result = matchedSentences.join(' ').substring(0, 8000);
  if (result.length > 0) {
    console.log(`[SEC Parser] ✅ extractRevenueNarrativeContext: ${result.length} chars`);
  }
  return result;
}

/**
 * Extract PP&E narrative context from SEC filing HTML.
 *
 * Fix 1: Searches for sentences containing PP&E/facility/asset keywords.
 * Returns up to 8,000 characters.
 * Assigned to result.ppeNarrativeContext in parseSECFiling().
 */
export function extractPPENarrativeContext(html: string): string {
  const $ = cheerio.load(html);
  const ppeKeywords = [
    'property', 'plant', 'equipment', 'pp&e', 'long-lived', 'fixed assets',
    'facility', 'facilities', 'manufacturing', 'warehouse', 'data center',
    'capital expenditure', 'capex', 'lease'
  ];
  ...
  function scanText(text: string) {
    ...
    for (const sentence of sentences) {
      const lower = sentence.toLowerCase();
      if (ppeKeywords.some(kw => lower.includes(kw))) {
        const trimmed = sentence.trim();
        if (trimmed.length > 20 && !seen.has(trimmed)) {
          seen.add(trimmed);
          matchedSentences.push(trimmed);
  ...
  const result = matchedSentences.join(' ').substring(0, 8000);
  if (result.length > 0) {
    console.log(`[SEC Parser] ✅ extractPPENarrativeContext: ${result.length} chars`);
  }
  return result;
}
...
export async function parseSECFiling(ticker: string): Promise<ParsedSECData | null> {
  ...
  try {
    ...
    if (!cik) {
      ...
      result.failureCategory = 'cik_not_found';
    ...
    if (!filing) {
      result.parsingErrors.push(`Could not fetch filing for CIK ${cik}`);
      result.failureCategory = 'filing_not_found';
      console.log(`[SEC Parser] ❌ No filing found`);
      return result;
    }
    if (!filing.html) {
      result.parsingErrors.push(`Filing found but HTML is empty for CIK ${cik}`);
      result.failureCategory = 'html_empty';
      console.log(`[SEC Parser] ❌ Filing HTML is empty`);
      return result;
    }
    ...
    // Fix 1: Assign narrative context fields from HTML text
    console.log(`[SEC Parser] Step 4b-ext: Extracting narrative context fields...`);
    result.supplyChainNarrativeContext = extractSupplyChainNarrativeContext(html);
    result.revenueNarrativeContext = extractRevenueNarrativeContext(html);
    result.ppeNarrativeContext = extractPPENarrativeContext(html);
    if (result.supplyChainNarrativeContext) {
      result.sectionsFound.push('Supply Chain Narrative Context');
    }
    if (result.revenueNarrativeContext) {
      result.sectionsFound.push('Revenue Narrative Context');
    }
    if (result.ppeNarrativeContext) {
      result.sectionsFound.push('PPE Narrative Context');
    }
    ...
    try {
      ...
      if (llmResult.extractions.length > 0) {
        ...
        // Fix 2: Propagate LLM extraction context to narrative context fields if still empty
        if (!result.supplyChainNarrativeContext || result.supplyChainNarrativeContext.trim().length === 0) {
          const supplyContextParts = llmResult.extractions
            .filter(e => (e as { channel?: string; extractionType?: string }).channel === 'supply' ||
                         (e as { channel?: string; extractionType?: string }).extractionType === 'supplier')
            .map(e => (e as { context?: string }).context || '')
            .filter(c => c.length > 0);
          if (supplyContextParts.length > 0) {
            result.supplyChainNarrativeContext = supplyContextParts.join(' ').substring(0, 8000);
            console.log(`[SEC Parser] Fix 2: Populated supplyChainNarrativeContext from LLM (${result.supplyChainNarrativeContext.length} chars)`);
          }
        }

        if (!result.revenueNarrativeContext || result.revenueNarrativeContext.trim().length === 0) {
          const revenueContextParts = llmResult.extractions
            .filter(e => (e as { channel?: string; extractionType?: string }).channel === 'revenue' ||
                         (e as { channel?: string; extractionType?: string }).extractionType === 'revenue_segment')
            .map(e => (e as { context?: string }).context || '')
            .filter(c => c.length > 0);
          if (revenueContextParts.length > 0) {
            result.revenueNarrativeContext = revenueContextParts.join(' ').substring(0, 8000);
            console.log(`[SEC Parser] Fix 2: Populated revenueNarrativeContext from LLM (${result.revenueNarrativeContext.length} chars)`);
          }
        }

        if (!result.ppeNarrativeContext || result.ppeNarrativeContext.trim().length === 0) {
          const ppeContextParts = llmResult.extractions
            .filter(e => (e as { channel?: string; extractionType?: string }).channel === 'assets' ||
                         (e as { channel?: string; extractionType?: string }).extractionType === 'facility')
            .map(e => (e as { context?: string }).context || '')
            .filter(c => c.length > 0);
          if (ppeContextParts.length > 0) {
            result.ppeNarrativeContext = ppeContextParts.join(' ').substring(0, 8000);
            console.log(`[SEC Parser] Fix 2: Populated ppeNarrativeContext from LLM (${result.ppeNarrativeContext.length} chars)`);
          }
        }
      } else {
    ...
    // Fix 1: Set success failure category based on whether tables were found
    const tablesFound = result.revenueTableFound || result.ppeTableFound || result.debtTableFound;
    result.failureCategory = tablesFound ? 'parsing_success_with_tables' : 'parsing_success_no_tables';
    ...
    console.log(`[SEC Parser] Failure category: ${result.failureCategory}`);
    console.log(`[SEC Parser] ========================================\n`);
...
export const secFilingParser = {
  ...
  extractSupplyChainNarrativeContext,
  extractRevenueNarrativeContext,
  extractPPENarrativeContext,
};