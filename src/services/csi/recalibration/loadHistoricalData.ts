 ...
 * 
 * @module recalibration/loadHistoricalData
 */
...
class HistoricalDataLoader {
  ...
  private async loadGDELTSignals(): Promise<number> {
    const sampleSignals = [
      { detected_at: '2024-01-15T10:30:00Z', source_id: 'gdelt', target_country: 'CHN', actor_country: 'USA', event_type: 'trade_restriction', severity: 65, goldstein_scale: -5.0, title: 'US considers new export controls on AI chips to China', url: 'https://example.com/news1' },
      { detected_at: '2024-03-15T14:20:00Z', source_id: 'gdelt', target_country: 'CHN', actor_country: 'USA', event_type: 'tariff_announcement', severity: 75, goldstein_scale: -6.5, title: 'Biden announces 25% tariffs on Chinese EVs', url: 'https://example.com/news2' },
      { detected_at: '2024-06-20T09:15:00Z', source_id: 'gdelt', target_country: 'RUS', actor_country: 'EU', event_type: 'sanctions', severity: 70, goldstein_scale: -7.0, title: 'EU announces 14th sanctions package on Russia', url: 'https://example.com/news3' },
      { detected_at: '2024-10-07T06:00:00Z', source_id: 'gdelt', target_country: 'ISR', actor_country: 'PSE', event_type: 'military_action', severity: 95, goldstein_scale: -10.0, title: 'Major escalation in Gaza conflict', url: 'https://example.com/news4' },
      { detected_at: '2024-02-14T11:45:00Z', source_id: 'gdelt', target_country: 'UKR', actor_country: 'RUS', event_type: 'military_action', severity: 85, goldstein_scale: -8.5, title: 'Ukraine conflict intensifies', url: 'https://example.com/news5' },
      { detected_at: '2024-04-13T15:30:00Z', source_id: 'gdelt', target_country: 'IRN', actor_country: 'ISR', event_type: 'military_action', severity: 80, goldstein_scale: -8.0, title: 'Iran-Israel tensions escalate', url: 'https://example.com/news6' },
      { detected_at: '2024-07-19T08:00:00Z', source_id: 'gdelt', target_country: 'USA', actor_country: null, event_type: 'cyber_incident', severity: 60, goldstein_scale: -4.0, title: 'CrowdStrike outage causes global disruption', url: 'https://example.com/news7' },
      { detected_at: '2024-05-10T12:00:00Z', source_id: 'gdelt', target_country: 'CHN', actor_country: 'USA', event_type: 'trade_restriction', severity: 68, goldstein_scale: -5.5, title: 'China retaliates with rare earth export controls', url: 'https://example.com/news8' }
    ];

    const stmt = this.db.prepare(`
      INSERT OR IGNORE INTO historical_signals (
        detected_at, source_id, target_country, actor_country, 
        event_type, severity, goldstein_scale, title, url
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    for (const signal of sampleSignals) {
      try {
        stmt.run(
          ...
          signal.goldstein_scale, signal.title, signal.url
        );
        count++;
      } catch (error) {
